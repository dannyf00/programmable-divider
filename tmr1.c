...
#else
	//*use 0 for mplab simulation, 1 in actual code
	TMR1CS = 0;							//1 = count T1CKI; 0=count Fosc/4
#endif
	TMR1GE = 0;							//0->tmr1 is always on; 1->tmr1 is on when /T1G pin is low
	//T1GE = 0;							//tmr1 gate disabled
	T1CON = (T1CON & 0xcf) | ((prescaler & 0x03) << 4);	//set the prescaler
	nT1SYNC = 0;						//0->tmr1 is synchronized, 1->tmr1 asynchronuous
